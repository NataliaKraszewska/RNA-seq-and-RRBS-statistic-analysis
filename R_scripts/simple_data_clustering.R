...
#identyfikujemy geny szumowe, tutaj wybieramy geny o najbardziej dynamicznej eks
presji, które mają nawiększą wariancję i ekspresję
z_var <- apply(z, 1, var)
...
plot(1:20, sil, type = "b", pch = 19, xlab = "Number of clusters k", ylab="Avera
ge silhouette width")
...
kClustcentroids <- sapply(levels(factor(kClusters)), clust.centroid, scaledata, 
kClusters)
...
par(cex.axis=0.6)
p2 <- ggplot(Kmolten, aes(x=sample,y=value, group=cluster, colour=as.factor(clus
ter))) + 
  geom_point() + 
  geom_line() +
  xlab("Time") +
